...
void Student::view_attendence(string username_check) {
    string check,batch, semester, section, password, username, course_code;
    check = "Students_info.csv";
    ifstream fin(check, ios::in);
    bool c = false;
    while(!fin.eof()) {
    ...
    fin.close();
    cout << "Enter Course Code of Course you want to see attendance: " << endl;
    cin >> course_code;
    check = batch + "/" + "semester" + semester + "/sec-" + section + "/" + username + "/" + course_code + "/" + course_code + "-Attendance.csv";
    ifstream file(check, ios::in);
    string a = "";
